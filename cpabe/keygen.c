//  gcc ../symmetric/efp12_symmetric.c ../symmetric/pairing.c ../symmetric/miller.c setup.c utilities.c hash_map.c key_gen.c -o key_gen.out -lgmp -lelips -I/opt/ssl/include/ -L/opt/ssl/lib/ -lcrypto

//  gcc ../symmetric/pairing.c ../symmetric/miller.c ../symmetric/efp12_symmetric.c efp12_w_naf.c utilities.c hash_map.c tree.c setup.c key_gen.c encrypt.c decrypt.c test.c -o test.out -lgmp -lelips -I/opt/ssl/include/ -L/opt/ssl/lib/ -lcrypto

// gcc ./cpabe/keygen.c ./cpabe/common.c ./cpabe/policy_lang.c ./libbswabe-0.9/core.c ./libbswabe-0.9/misc.c -o keygen.out `pkg-config --cflags glib-2.0` `pkg-config --libs glib-2.0` -lgmp /usr/local/include/mcl/libmcl.a -lcrypto -lstdc++ -O3 -w

// ./keygen.out -o lhanh_priv_key pub_key master_key att1 att2 att3 att4

...
unserialize_element( GByteArray* b, int* offset, mclBnGT e )
{
	...
	*offset += len;
	...
	mclBnGT_deserialize(&e, buf, sizeof(buf));
	free(buf);
}
...
void unserialize_G1(GByteArray* b, int* offset, mclBnG1 e){
	...
	memcpy(buf, b->data + *offset, len);
	*offset += len;
	mclBnG1_deserialize(&e, buf, sizeof(buf));
	free(buf);
}
...
void unserialize_G2(GByteArray* b, int* offset, mclBnG2 e){
	...
	memcpy(buf, b->data + *offset, len);
	*offset += len;
	mclBnG2_deserialize(&e, buf, sizeof(buf));
	free(buf);
}
...
void unserialize_Fr(GByteArray* b, int* offset, mclBnFr e){
	...
	memcpy(buf, b->data + *offset, len);
	*offset += len;
	mclBnFr_deserialize(&e, buf, sizeof(buf));
	free(buf);
}
...
serialize_policy(GByteArray* b, bswabe_policy_t* p )
{
	...
	if(p->children->len == 0)
	{
		...
		for(i = 0; i < p->children->len; i++ ){
			printf("serialize_policy: %d\n", i);
			bswabe_policy_t* p1 = g_ptr_array_index(p->children, i);
			printf("p1->children->len: %s\n", p1->children->len);

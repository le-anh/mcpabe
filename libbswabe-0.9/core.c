...
void bswabe_setup(bswabe_pub_t **pub, bswabe_msk_t **msk)
{
	...
	*msk = malloc(sizeof(bswabe_msk_t));
	...
	char buf[1600];
	mclBnFr_getStr(buf, 1600, &alpha, 16);
	printf("core.c -> bswabe_setup -> alpha: %s\n", buf);
	mclBnFr_getStr(buf, 1600, &(*msk)->beta, 16);
	printf("core.c -> bswabe_setup -> (*msk)->beta: %s\n", buf);
	mclBnG2_getStr(buf, 1600, &(*msk)->g_alpha, 16);
	printf("core.c -> bswabe_setup -> (*msk)->g_alpha: %s\n", buf);

	mclBnG1_getStr(buf, 1600, &(*pub)->g, 16);
	printf("core.c -> bswabe_setup -> (*pub)->g: %s\n", buf);
	mclBnG2_getStr(buf, 1600, &(*pub)->gp, 16);
	printf("core.c -> bswabe_setup -> (*pub)->gp: %s\n", buf);
	mclBnG1_getStr(buf, 1600, &(*pub)->h, 16);
	printf("core.c -> bswabe_setup -> (*pub)->h: %s\n", buf);
	mclBnGT_getStr(buf, 1600, &(*pub)->g_hat_alpha, 16);
	printf("core.c -> bswabe_setup -> (*pub)->g_hat_alpha: %s\n", buf);
}
							...
							char **attributes)
{
	...
	mclBnFr_getStr(buf, 1600, &msk->beta, 16);
	printf("core.c -> bswabe_keygen -> (*msk)->beta: %s\n", buf);
	mclBnG2_getStr(buf, 1600, &msk->g_alpha, 16);
	printf("core.c -> bswabe_keygen -> (*msk)->g_alpha: %s\n", buf);

	mclBnG1_getStr(buf, 1600, &pub->g, 16);
	printf("core.c -> bswabe_keygen -> (*pub)->g: %s\n", buf);
	mclBnG2_getStr(buf, 1600, &pub->gp, 16);
	printf("core.c -> bswabe_keygen -> (*pub)->gp: %s\n", buf);
	mclBnG1_getStr(buf, 1600, &pub->h, 16);
	printf("core.c -> bswabe_keygen -> (*pub)->h: %s\n", buf);
	mclBnGT_getStr(buf, 1600, &pub->g_hat_alpha, 16);
	printf("core.c -> bswabe_keygen -> (*pub)->g_hat_alpha: %s\n", buf);
...
dec_leaf_flatten(mclBnGT *r, mclBnFr exp, bswabe_policy_t *p, bswabe_prv_t *prv, bswabe_pub_t *pub)
{
	...
	// char buf[1600];
	// mclBnGT_getStr(buf, 1600, &(p->c), 16);
	// printf(" core.c -> dec_leaf_flatten -> &p->c: %s\n", buf);
	// mclBnGT_getStr(buf, 1600, &(c->d), 16);
	// printf(" core.c -> dec_leaf_flatten -> &c->d: %s\n", buf);
	// mclBnGT_getStr(buf, 1600, &s, 16);
	// printf(" core.c -> dec_leaf_flatten -> s: %s\n", buf);
	// mclBnGT_getStr(buf, 1600, &t, 16);
	// printf(" core.c -> dec_leaf_flatten -> t: %s\n", buf);
...
dec_node_flatten(mclBnGT *r, mclBnFr exp, bswabe_policy_t *p, bswabe_prv_t *prv, bswabe_pub_t *pub)
{
	...
	char buf[1600];
	mclBnGT_getStr(buf, 1600, r, 16);
	printf(" core.c -> dec_node_flatten -> r: %s\n", buf);
}
...
dec_flatten(mclBnGT *r, bswabe_policy_t *p, bswabe_prv_t *prv, bswabe_pub_t *pub)
{
	...
	char buf[1600];
	mclBnGT_getStr(buf, 1600, r, 16);
	printf(" core.c -> dec_flatten -> r (t1): %s\n", buf);

	dec_node_flatten(r, one, p, prv, pub);

	mclBnGT_getStr(buf, 1600, r, 16);
	printf(" core.c -> bswabe_dec -> r (t2): %s\n", buf);
...
bswabe_dec(bswabe_pub_t *pub, bswabe_prv_t *prv, bswabe_cph_t *cph, mclBnGT *m)
{
	...
	char buf[1600];
	mclBnGT_getStr(buf, 1600, &t, 16);
	printf(" core.c -> bswabe_dec -> t: %s\n", buf);

	// MCL
	mclBnGT_mul(m, &cph->cs, &t);

	// mclBnGT_getStr(buf, 1600, m, 16);
	// printf(" core.c -> bswabe_dec -> m: %s\n", buf);

	// mclBnGT_getStr(buf, 1600, &cph->c, 16);
	// printf(" core.c -> bswabe_dec -> &cph->c: %s\n", buf);

	// mclBnGT_getStr(buf, 1600, &prv->d, 16);
	// printf(" core.c -> bswabe_dec -> &prv->d: %s\n", buf);
